...
/**
 * Tool: Semantic search across all content using RAG
 */
const semanticSearchTool = createTool({
  name: "semanticSearch",
  description:
    "Search across all content (tasks, notes, voice notes, files) using semantic/meaning-based search. Use this when the user asks a question about their content or wants to find related information.",
  args: v.object({
    query: v.string(),
    limit: v.optional(v.number()),
  }),
  handler: async (ctx, args) => {
    try {
      // Import RAG dynamically since it requires "use node"
      ...
      return {
        ...
        matchCount: results.entries.length,
      };
    } catch (error) {
      ...
      return {
        ...
        entries: [],
        matchCount: 0,
        error: "Semantic search is not available. Using keyword search instead.",
      };
    }
  },
});
