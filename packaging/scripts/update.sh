...
if [[ -n ${previous} ]]; then
  ...
  previous_health=$("${previous}/bin/kriyan-node" --process-health-config "${etc_root}/node.json" || true)
...
if ! "${systemctl_cmd}" restart kriyan-node || \
  ...
  if [[ -n ${previous} ]]; then
    ...
    printf 'KRIYAN_RELEASE_VERSION=%s\n' "${previous_version}" >"${etc_root}/release.env"
    "${systemctl_cmd}" restart kriyan-node
    "${systemctl_cmd}" is-active --quiet kriyan-node
    rollback_at=$(date +%s%3N)
    "$(dirname "$0")/wait-for-health.sh" \
      "${previous}/bin/kriyan-node" "${etc_root}/node.json" "${previous_version}" \
